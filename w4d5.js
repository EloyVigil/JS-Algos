...
function canBuildS1FromS2(s1, s2) {
    ...
    for(let i = 0; i < s2.length; i++){
        if(!newObj[s2[i].toLocaleLowerCase()]){
            newObj[s2[i].toLocaleLowerCase()] = 1;
        } else {
            newObj[s2[i].toLocaleLowerCase()]++
        }
    }
    for(let j = 0; j < s1.length; j++){
        if(!newObj[s1[j].toLocaleLowerCase()]){
            return false;
        } else {
            newObj[s1[j].toLocaleLowerCase()]--
        }
    }
    return true;
}
